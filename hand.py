...
class Hand:
    ...
    def render_winner(self):
        # Send the text and color basend on the winner and print to screen
        if self.dealer.determined_winner is not None:
            ...
            rotated_rect = rotated_surface.get_rect(text_surface, self.win_rotation_angle)
...
class Dealer():
    def __init__(self, players, flop) -> None:
        ...
        if self.can_deal:
            ...
            if self.current_player_index == 0:
                if len(current_player.cards) == 1:
                    current_player.cards[0].position = (P1_C1[0], current_player[0].card_y)
                elif len(current_player.cards) == 2:
                    current_player.cards[1].position = (P1_C2[0], current_player[1].card_y)
                self.animating_card = current_player.cards[-1]
            # Card Two of two
            elif self.current_player_index == 1:
                if len(current_player.cards) == 1:
                    current_player.cards[0].position = ((P2_C1[0] - current_player.cards[0].card_surf.get_width()), current_player.cards[0].card_y)
                elif len(current_player.cards) == 2:
                    current_player.cards[1].position = ((P2_C2[0] - current_player.cards[1].card_surf.get_width()), current_player.cards[1].card_y)
                self.animating_card = current_player.cards[-1]
    ...
    def deal_flop(self):
        ...
        # Three flop cards in above set locations; remove from desk; flop cooldown
        ...
        # Print length of deck after card is dealt for trobleshooting
    ...
    def eval_hand(self, hand):
        ...
        if trips: return(6 if pairs else 3), trips, pairs, value_dict
    ...
    def update(self):
        ...
        if self.dealt_cards == (self.num_player * 2) and (not self.animating_card or self.animating_card.animation_complete):
            self.can_deal_flop = True
            self.deal_flop()
        ...
        if self.dealt_cards == ((self.num_players * 2) + 3) and self.determined_winner is None:
            eval_cards = [card_id.id for card_id in self.players.list[0].cards] + [card_id.id for card_id in self.flop.cards] + [card_id.id for card_id in self.flop.cards] + [card_id.id for card_id in self.flop.cards] + [card_id.id for card_id in self.players.list[1].cards]
